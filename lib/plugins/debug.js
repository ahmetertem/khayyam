...
function Debug(config, app) {
  ...
  app.markets.on('market_pair_added', function () {
  ...
  app.markets.on('market_pair_tick', function (market) {
    self.debug('market_pair_tick ' + market.name)
  })
  app.markets.on('market_pair_public_trades_set', function (market, pair, count) {
    self.debug('market_pair_public_trades_set ' + market.name + ' pair ' + pair.currency2 + '/' + pair.currency2 + ' => ' + count)
  })
...
Debug.prototype.name = 'Debug'
Debug.prototype.version = '0.0.4'
