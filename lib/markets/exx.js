...
function EXX(apiKey, apiSecret) {
  ...
  this._tradingUrl = 'https://trade.exx.com/api'
  ...
  this._getMyBalances = function () {
    var self = this
    var response = self.query('getBalance');
  ...
  this.query = function (command, args) {
    ...
    args.accesskey = self._apiKey
    var postData = 'accesskey=' + self._apiKey + '&nonce=' + args.nonce
    ...
    var ret
    var url = self._tradingUrl + '/' + command + '?' + postData + '&signature=' + sign
