...
function poloniex(api_key, api_secret) {
		...
		if (pair_string === 'all') {
			_.each(self.pairs, function(pair, pair_index) {
				...
				_.each(response[pair_string], function(order) {
					var new_order = new Order(order.orderNumber, order.rate, order.amount, order.type, new Date(order.date), pair.currency2_decimal);
					list.push(new_order);
				});
			...
			_.each(response, function(order) {
				var new_order = new Order(order.orderNumber, order.rate, order.amount, order.type, new Date(order.date), args.currency2_decimal);
				list.push(new_order);
			});
	...
	this._getDepths = function(args) {
		...
		if (pair_string === 'all') {
			_.each(self.pairs, function(pair, pair_index) {
				...
				_.each(response[pair_string], function(order) {
					...
					_.each(response.asks, function(depth) {
						list.push(new Order(null, depth[0], depth[1], null, null, pair.currency2_decimal));
					});
			...
			var list = [];
			_.each(response.asks, function(depth) {
				list.push(new Order(null, depth[0], depth[1], null, null, args.currency2_decimal));
			});
	...
	this._getPublicTrades = function(args) {
		...
		if (pair_string === 'all') {
			...
			_.each(response, function(trade) {
				var new_order = new Order(null, trade.rate, trade.amount, trade.type, new Date(trade.date), args.currency2_decimal);
				list.push(new_order);
			});
		...
		if (pair_string === 'all') {
			_.each(self.pairs, function(pair, pair_index) {
				...
				_.each(response[pair_string], function(trade) {
					var new_order = new Order(trade.globalTradeID, trade.rate, trade.amount, trade.type, new Date(trade.date), pair.currency2_decimal);
					list.push(new_order);
				});
			...
			var list = [];
			_.each(response, function(trade) {
				var new_order = new Order(trade.globalTradeID, trade.rate, trade.amount, trade.type, new Date(trade.date), args.currency2_decimal);
				list.push(new_order);
			});
