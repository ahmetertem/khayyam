...
function poloniex(api_key, api_secret) {
	...
	this._getBalances = function() {
		...
		let currencies = self.getMyPairCurrencies();
	...
	this._getOrders = function() {
		...
		_.each(self.pairs, function(value, key) {
			let pair_string = value.currency1.toUpperCase() + '_' + value.currency2.toUpperCase();
			self.pairs[key].my_orders = [];
			_.each(response[pair_string], function(order) {
				let new_order = new Order(order.orderNumber, order.rate, order.amount, order.type, new Date(order.date), value.currency2_decimal);
				self.pairs[key].my_orders.push(new_order);
			});
		});
	...
	this._getDepths = function(pair, args) {
		...
		pair = args.currency1.toUpperCase() + '_' + args.currency2.toUpperCase();
		let response = JSON.parse(self.public({
		...
		self.pairs[args.pair_index].depth_asks = list;
		//
		// bids
		//
		var list = [];
		...
		self.pairs[args.pair_index].depth_bids = list;
		console.log(self.pairs[args.pair_index]);
	};
