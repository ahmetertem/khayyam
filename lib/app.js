...
module.exports = {
  ...
  readPlugins: function () {
    var self = this
    self.plugins = JSON.parse(fs.readFileSync(path.join(self.applicationDataFolder, 'plugins.json'), 'utf-8'))
  },
  writePlugins: function () {
    var self = this
    fs.writeFileSync(path.join(self.applicationDataFolder, 'plugins.json'), JSON.stringify(self.plugins))
  },
  init: function () {
    var self = this
    self.markets.init()
