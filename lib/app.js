...
module.exports = {
  ...
  checkDatabaseConnection: function () {
    ...
    self.database.query(testTable.create().ifNotExists().toQuery().text, function (err) {
      if (err !== null) {
  ...
  init: function () {
    ...
    if(self.settings.password === null) {
      self.log('You didn\'t set any master password. If you do not set any password, your API secrets will be readable!')
      self.log('You may set or change password with `passwd` command')
    }
