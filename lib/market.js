		...
		if (_.isNull(args.currency1)) {
			_.each(self.pairs, function(pair, pair_index) {
				self._getMyOrders({
					...
					currency2_decimal: pair.currency2_decimal
				});
			});
		} else {
			...
			if (_.isNull(args.currency2_decimal)) {
				args.currency2_decimal = self.pairs[args.pair_index].currency2_decimal;
			}
