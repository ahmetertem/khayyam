...
function Markets() {
  ...
  this.load = function (password) {
    var self = this
    var json = JSON.parse(fs.readFileSync(this.settingsFilePath, 'utf-8'))
    _.forEach(json, function (market) {
      var apiKey = market.apiKey
      var apiSecret = market.apiSecret
      if(password !== null) {
        apiKey = lib.aesDecrypt(apiKey, password)
        apiSecret = lib.aesDecrypt(apiSecret, password)
      }
      if (self.add(market.name, apiKey, apiSecret, market.extraParams, {
        save: false,
        initialize: false
      })) {
  ...
  this.save = function () {
    ...
    _.forEach(this.markets, function (market) {
      ...
      list.push({
        ...
        extraParams: market.extraParams,
        pairs: pairs
      })
    })
  ...
  this.add = function (marketName, apiKey, apiSecret, extraParams, args) {
    args = _.merge({
      save: true,
      initialize: true
    }, args)
    ...
    var self = this
    var market_ = new Market(apiKey, apiSecret, extraParams)
    ...
    }).on('pair_public_trades_set', function (pair, count) {
      self.emit('market_pair_public_trades_set', this, pair, count)
    }).on('pair_my_orders_set', function (pair, count) {
      this.emit('market_pair_my_orders_set', this, pair, count)
    })
    //
    ...
    market_.index = this.markets.length
    if (args.initialize) {
      market_.initialize()
    }
