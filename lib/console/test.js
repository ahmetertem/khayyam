...
function draw() {
    ...
    _.forEach(market.pairs, function (pair, pi) {
      ...
      if(mi == market_index && pi == pair_index) {
        ...
        while(oi < max) {
          ...
          asks.push([order.rate, order.amount, order.total])
          oi++
        }
        oi = 0
        ...
        while(oi < max) {
          ...
          bids.push([order.rate, order.amount, order.total])
          oi++
        }
        asks = arrayPrefixate(asks, 1)
        asks = arrayPrefixate(asks, 2)
        bids = arrayPrefixate(bids, 1)
  ...
  if(app_.markets.markets.length > 1) {
    out += chalk.redBright('M') + ': Change market.' + chalk.gray('(Current: '+ app_.markets.markets[market_index].name + ')') + nl
  }
  if(app_.markets.markets[market_index].pairs.length > 1) {
    out += chalk.redBright('P') + ': Change pair.' + chalk.gray('(Current: ' + app_.markets.markets[market_index].pairs[pair_index].currency2.toUpperCase() + '/' + app_.markets.markets[market_index].pairs[pair_index].currency1.toUpperCase() + ')') + nl
  }
  if(interval > 1000) {
    out += chalk.redBright('[') + ': Decrease interval.' + chalk.gray('(Current: ' + (interval / 1000) + 's)') + nl
  }
  if(interval < 10000) {
    out += chalk.redBright(']') + ': Increase interval.' + chalk.gray('(Current: ' + (interval / 1000) + 's)') + nl
  }
  out += nl
...
function arrayPrefixate(arr, index) {
  var ml = 0
  var tl
...
function headerTable(table_string, title) {
  ...
  line = def.bodyLeft
  line += _.repeat(' ', leftSpace)
  line += chalk.yellow(title)
  line += _.repeat(' ', rightSpace)
  line += def.bodyRight
  lines.push(line)
