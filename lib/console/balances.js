...
module.exports = function (vorpal, app) {
  var ans = function (marketIndex, hidezero) {
    ...
    _.each(currencies, function (currency) {
      if (!hidezero || (hidezero && app.markets.markets[marketIndex].balances_all[currency] > 0)) {
        data.push([currency.toUpperCase(), app.markets.markets[marketIndex].balances_in_pockets[currency], app.markets.markets[marketIndex].balances_on_orders[currency], app.markets.markets[marketIndex].balances_all[currency]])
      }
    })
    if (data.length === 0) {
