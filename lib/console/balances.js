  ...
  var ans = function (marketIndex, hidezero) {
    ...
    _.each(currencies, function (currency) {
      if (!hidezero || (hidezero && app.markets.markets[marketIndex].balancesAll[currency] > 0)) {
        ...
        n.push(app.markets.markets[marketIndex].balancesInPockets[currency])
        n.push(app.markets.markets[marketIndex].balancesOnOrders[currency])
        n.push(app.markets.markets[marketIndex].balancesAll[currency])
        data.push(n)
      }
    })
    if (data.length === 0) {
